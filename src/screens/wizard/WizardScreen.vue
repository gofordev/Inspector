<template>
  <div>
    <md-toolbar class="md-primary nav-bar wizard" position="static">
    ...
    <div class="main-wrapper wizard">
      <div class="df">
          <div class="f2">
              <md-card class="paper product-info-only">
                  <h2>№ {{check.check_number}}</h2>
                  <div class="bold">{{check.check_title}}</div>
                  <span class="product-title">{{check.product_title}}</span>
              </md-card>
          </div>
          <div class="f2">
      ...
      <div class="df">
          ...
          <div class="f2">
              <md-card class="paper verification-comment">
                  <h2>Комментарий</h2>
                  <div class="paper-content">{{featuresData.get(feature.feature_id).status.note || feature.note}}</div>
    ...
    <md-dialog class="dialog-comment" :md-active.sync="openComments">
        <md-dialog-title class="dialog-title"><h2 style="color:#000000">Комментарий</h2></md-dialog-title>
        <md-dialog-content class="dialog-content">
          <!-- <md-field>
            <md-textarea rowsMax="4" v-model="note" />
          </md-field> -->
          <md-field>
            <md-input
              id="noter"
...
import { finalPage } from '../../constants/config';
...
export default class WizardScreen extends Vue {
  ...
  get check() {
    return state.checks.list.find(check => check.check_id == this.$route.params.checkId)
  }
    ...
    if (this.position + 1 === this.featuresData.count()) {
        ...
        return this.$router.push({
            ...
            params: {
                ...
                operationId: this.operation.operation_id
            }
        })
    }
  ...
  addFile(result: string) {
    ...
    this.featuresData = this.featuresData.update(feature.feature_id, (_feature: RequestFeature) => {
      ...
      // sendPhotoAction(feature.feature_id, photo, Mode.check);
